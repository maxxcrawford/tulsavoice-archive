...
{ // disallow a blank field
  if(fld.disabled) return true; // hard to require something the user can't interact with
  if (fld.nodeName == 'SELECT' && fld.options.length == 0) return true; // hard to require something the user can't interact with, again
  if(!fld.value.length)
